    ...
    <div>
      ...
      {selectedService && (
        <div className="bg-card border border-primary/25 rounded-card shadow-card p-5 mb-6 animate-fade-in">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4 text-sm"><div><p className="text-xs font-semibold text-text-secondary mb-1">Descripción</p><p className="text-text-primary">{selectedService.description}</p></div><div><p className="text-xs font-semibold text-text-secondary mb-1">Función principal</p><p className="text-text-primary">{selectedService.mainFunction}</p></div></div>
        </div>
      )}
